-- Repair balance RPC exposure for staging and future environments.
-- This migration is idempotent and safe to re-run on a database that already has the functions.
...
GRANT EXECUTE ON FUNCTION public.add_balance(uuid, numeric, text, text) TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.deduct_balance(uuid, numeric, text, text) TO anon, authenticated, service_role;
