...
export function WorkspaceHeader({
  ...
  return (
    <div className="rounded-[28px] border border-border bg-card px-4 py-5 shadow-sm sm:px-5 md:px-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="min-w-0">
          ...
          <h1 className="mt-3 text-xl font-bold text-foreground sm:text-2xl md:text-3xl">{title}</h1>
        ...
        {!!stats.length && (
          <div className="grid grid-cols-3 gap-2 rounded-2xl bg-muted/60 p-2 text-center sm:gap-3 sm:p-3">
            {stats.map((stat) => (
...
export function WorkspaceShell({
  ...
  return (
    <div className={cn("grid gap-4 lg:gap-5 xl:gap-6", sidebarWidthClassName)}>
      <aside className="xl:sticky xl:top-24 xl:self-start">{sidebar}</aside>
      <section className="min-w-0 space-y-4 lg:space-y-5 xl:space-y-6">{content}</section>
    </div>
  );
}
